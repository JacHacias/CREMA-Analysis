...
def _refine_with_linear_background(x, counts, popt0):
    ...
    Voigt fit. The pedestal under these lines is sloped; a windowed flat-background Voigt
    cannot capture it and the centroid is biased. Returns a 5-parameter Voigt-compatible
    popt (background = linear term at the center) plus its uncertainties, or None if the
    refinement fails or moves the center implausibly far."""
    ...
    # Reject implausible jumps (full-range fit pulled away by other structure).
