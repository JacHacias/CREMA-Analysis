...
- The neutralizer correction is explicit and opt-in through `neutralization`.
  Use `neutralization="none"` for the historical charged-ion beta correction, `neutralization="electron_capture"` to conserve the incoming ion momentum while changing to the neutral sulfur mass after electron pickup, or `neutralization="sodium_charge_exchange"` to use a simple collinear two-body sulfur/sodium charge-exchange kinematic model.
...
- The sodium neutralizer mass defaults to `SODIUM_MASS_U = 22.9897692820`. You can set `sodium_velocity_m_s` if you want to include a directed sodium vapor velocity; the default is stationary sodium in the lab frame.
